...
CWindow::CWindow( const CSettings &settings, const CFileSystem &filesystem, const std::string &windowTitle, const std::string &iconPath )
{
	...
	if( !iconPath.empty() )
	{
		...
		if( image )
		{
			SDL_Surface *gameIcon = SDL_CreateRGBSurfaceFrom( const_cast< void* >( reinterpret_cast< const void* >( image->RawPixelData() ) ), image->Size().width, image->Size().height, image->BPP(), image->Pitch(), 0x000000FF, 0x0000FF00, 0x00FF0000, 0xFF000000 );
